...
#[test]
fn clears_unacked_tun_tcp_server_payload_when_client_payload_acknowledges_latest_server_sequence() {
    let syn_packet = ipv4_packet(
        6,
        "10.7.0.2",
        "93.184.216.34",
    ...
    assert!(sessions
        ...
        .server_unacked_payload
        .is_some());
    let next_payload_packet = ipv4_packet(
        6,
        "10.7.0.2",
        "93.184.216.34",
        &tcp_segment(49152, 443, 16, 1009, 0x0018, 0x4000, &[], b"more"),
    );
    let next_payload =
        parse_tun_tcp_segment(&next_payload_packet).expect("parse next client payload");

    sessions
        .accept_client_payload(&next_payload)
        .expect("accept next client payload")
        .expect("next payload frame");
    ...
    let stale_ack_packet = ipv4_packet(
        ...
        "10.7.0.2",
        "93.184.216.34",
        &tcp_segment(49152, 443, 20, 1001, 0x0010, 0x4000, &[], b""),
    );
    let stale_ack = parse_tun_tcp_segment(&stale_ack_packet).expect("parse stale ACK segment");
    assert!(sessions
        .retransmit_server_payload(&stale_ack)
        .expect("try retransmit acknowledged server payload")
        .is_none());
}
...
#[test]
fn acknowledges_tun_tcp_client_ack_with_stale_known_server_ack_and_keeps_session_active() {
    let syn_packet = ipv4_packet(
        6,
        "10.7.0.2",
        "93.184.216.34",
    ...
    let invalid_ack_packet = ipv4_packet(
        ...
        "10.7.0.2",
        "93.184.216.34",
        &tcp_segment(49152, 443, 16, 1000, 0x0010, 0x4000, &[], b""),
    );
    let invalid_ack =
        parse_tun_tcp_segment(&invalid_ack_packet).expect("parse invalid ACK segment");
    assert!(sessions
        .acknowledge_client_ack(&invalid_ack)
        .expect("try invalid client ACK")
        .is_none());
    ...
    let session = sessions
        .acknowledge_client_ack(&stale_ack)
        .expect("ack client ACK")
        .expect("client ACK session");

    assert_eq!(session.client_next_sequence_number, 16);
    assert_eq!(session.server_next_sequence_number, 1009);
    assert_eq!(
        sessions.get(&key).expect("stored session").last_activity_at,
        session.last_activity_at
    );
    let pruned = sessions.prune_idle(
        session.last_activity_at + Duration::from_secs(4),
        Duration::from_secs(5),
    );
    assert!(
        pruned.is_empty(),
        "known ACK-only traffic should keep the TCP session active"
    );
...
fn processes_tun_tcp_session_steps_with_relay_callbacks_and_response_packets() {
    ...
    assert_eq!(stale_ack_step.response_packets().len(), 0);
    let TunTcpSessionStep::ClientAck { session } = stale_ack_step else {
        panic!("expected stale ACK to be accepted without retransmission");
    };
    assert_eq!(session.client_next_sequence_number, 20);
    assert_eq!(session.server_next_sequence_number, 1009);
    assert!(session.server_unacked_payload.is_none());
    assert_eq!(
        relay.client_payloads,
        vec![b"GET /".to_vec(), b"more".to_vec()],
...
#[test]
fn retransmits_tun_tcp_server_fin_until_client_acknowledges_close() {
    let syn_packet = ipv4_packet(
        6,
        "10.7.0.2",
        "93.184.216.34",
    ...
    let retransmit_step =
        process_tun_tcp_session_segment(&mut sessions, &stale_ack, &mut relay, 1000, 0x2000)
            .expect("process stale ACK after server FIN");

    assert_eq!(retransmit_step.response_packets().len(), 1);
    let TunTcpSessionStep::ServerCloseRetransmission { response } = retransmit_step else {
        panic!("expected server FIN retransmission");
    };
    assert_eq!(response.sequence_number, 1001);
    assert_eq!(response.acknowledgment_number, 16);
    let fin = parse_tun_tcp_segment(&response.packet).expect("parse retransmitted server FIN");
    assert_eq!(fin.sequence_number, 1001);
    assert_eq!(fin.acknowledgment_number, 16);
    assert!(fin.flags.fin());
    assert!(fin.flags.ack());
    assert!(fin.payload.is_empty());
    assert_eq!(
        relay.closed_sessions.len(),
        1,
        "server FIN retransmission must not close the relay again"
    );

    let final_ack_step =
        process_tun_tcp_session_segment(&mut sessions, &final_ack, &mut relay, 1000, 0x2000)
            .expect("process final ACK after server FIN");

    assert!(final_ack_step.response_packets().is_empty());
    let TunTcpSessionStep::ServerCloseAcknowledged { session } = final_ack_step else {
        panic!("expected server FIN final ACK");
    };
    assert_eq!(session.client_next_sequence_number, 16);
...
#[test]
fn acknowledges_tun_tcp_post_close_client_fin_payload_without_reset() {
    let syn_packet = ipv4_packet(
        6,
        "10.7.0.2",
        "93.184.216.34",
    ...
    let final_ack = parse_tun_tcp_segment(&final_ack_packet).expect("parse final ACK segment");
    let late_fin_payload_packet = ipv4_packet(
        6,
        "10.7.0.2",
        "93.184.216.34",
        &tcp_segment(49152, 443, 16, 1002, 0x0019, 0x4000, &[], b"tail"),
    );
    let late_fin_payload =
        parse_tun_tcp_segment(&late_fin_payload_packet).expect("parse late FIN payload segment");
    let mut sessions = TunTcpSessionTable::new();
    let mut relay = FakeTunTcpSessionRelay::with_server_reads(vec![TunTcpServerRead::Closed]);
    ...
    let late_fin_payload_step =
        process_tun_tcp_session_segment(&mut sessions, &late_fin_payload, &mut relay, 1000, 0x2000)
            .expect("process late FIN payload");

    let TunTcpSessionStep::ServerCloseClientFinAck { response } = late_fin_payload_step else {
        panic!("expected post-close FIN payload ACK");
    };
    assert_eq!(response.sequence_number, 1002);
    assert_eq!(response.acknowledgment_number, 21);
    let ack = parse_tun_tcp_segment(&response.packet).expect("parse post-close FIN payload ACK");
    assert_eq!(ack.sequence_number, 1002);
    assert_eq!(ack.acknowledgment_number, 21);
    assert!(ack.flags.ack());
    assert!(!ack.flags.fin());
    assert!(!ack.flags.rst());
    assert!(ack.payload.is_empty());
    ...
    let duplicate_late_fin_payload_step =
        process_tun_tcp_session_segment(&mut sessions, &late_fin_payload, &mut relay, 1000, 0x2000)
            .expect("process duplicate late FIN payload");

    let TunTcpSessionStep::ServerCloseClientFinAck { response } = duplicate_late_fin_payload_step
    else {
        panic!("expected duplicate post-close FIN payload ACK");
    };
    assert_eq!(response.sequence_number, 1002);
    assert_eq!(response.acknowledgment_number, 21);
    let duplicate_ack = parse_tun_tcp_segment(&response.packet)
        .expect("parse duplicate post-close FIN payload ACK");
    assert_eq!(duplicate_ack.sequence_number, 1002);
    assert_eq!(duplicate_ack.acknowledgment_number, 21);
    assert!(duplicate_ack.flags.ack());
...
#[test]
fn acknowledges_client_fin_with_stale_known_server_ack_and_retransmits_unacked_payload() {
    let syn_packet = ipv4_packet(
        6,
        "10.7.0.2",
        "93.184.216.34",
    ...
    let TunTcpSessionStep::ServerPayloadRetransmission { response } = retransmit_step else {
        panic!("expected server payload retransmission");
    };
    assert_eq!(response.sequence_number, 1001);
    assert_eq!(response.acknowledgment_number, 12);
    assert_eq!(response.payload, b"HTTP/1.1");
...
#[test]
fn duplicate_client_fin_with_latest_server_ack_polls_server_payload() {
    let syn_packet = ipv4_packet(
        6,
        "10.7.0.2",
        "93.184.216.34",
...
fn acknowledges_fin_payload_with_stale_known_server_ack_and_retransmits_unacked_payload() {
    ...
    assert_eq!(frame.payload, b"POST");
    assert_eq!(response.sequence_number, 1009);
    assert_eq!(response.acknowledgment_number, 16);
    ...
    let retransmit_step =
        process_tun_tcp_session_segment(&mut sessions, &stale_ack, &mut relay, 1000, 0x2000)
            .expect("process stale ACK after FIN payload");
...
fn registry_tun_tcp_session_relay_writes_fin_payload_before_close_once() {
    ...
    assert_eq!(summary.tcp_server_close_markers_peak, 1);
    assert_eq!(summary.tcp_post_close_markers_peak, 0);
    assert_eq!(device.writes.len(), 4);
...
fn tun_packet_loop_with_tcp_session_relay_retransmits_server_payload_on_duplicate_ack() {
    ...
    assert_eq!(summary.tcp_post_close_markers_open, 0);
    assert_eq!(summary.tcp_sessions_peak, 1);
    assert_eq!(summary.tcp_server_close_markers_peak, 0);
    assert_eq!(summary.tcp_post_close_markers_peak, 0);
    assert_eq!(device.writes.len(), 4);
    let server_payload =
...
#[test]
fn tun_packet_loop_clears_server_payload_retransmit_after_client_payload_acknowledges_latest_server_sequence(
) {
    let packets = vec![
        ipv4_packet(
            6,
    ...
    .expect("run TUN loop with acknowledged TCP server payload");
    ...
    assert_eq!(server_payload.acknowledgment_number, 16);
    assert_eq!(server_payload.payload, b"HTTP/1.1");
    let next_payload_ack =
        parse_tun_tcp_segment(&device.writes[3]).expect("parse next client payload ACK");
    assert_eq!(next_payload_ack.sequence_number, 1009);
    assert_eq!(next_payload_ack.acknowledgment_number, 20);
    assert!(next_payload_ack.payload.is_empty());
    assert_eq!(
        relay.client_payloads,
        vec![b"GET /".to_vec(), b"more".to_vec()]
    );
    let key = TunTcpSessionKey {
        source_ip: "10.7.0.2".parse().expect("valid source IP"),
        source_port: 49152,
        destination_ip: "93.184.216.34".parse().expect("valid destination IP"),
        destination_port: 443,
    };
    let stored = sessions.get(&key).expect("session should remain");
    assert!(stored.server_unacked_payload.is_none());
}

#[test]
fn tun_packet_loop_retransmits_server_fin_and_absorbs_final_ack_without_reset() {
    let packets = vec![
        ipv4_packet(
            6,
            "10.7.0.2",
    ...
    .expect("run TUN loop with server FIN retransmission and final ACK");
    ...
    assert!(server_fin.flags.fin());
    assert!(server_fin.flags.ack());
    let retransmitted_fin =
        parse_tun_tcp_segment(&device.writes[3]).expect("parse retransmitted server FIN packet");
    assert_eq!(retransmitted_fin.sequence_number, 1001);
    assert_eq!(retransmitted_fin.acknowledgment_number, 16);
    assert!(retransmitted_fin.flags.fin());
    assert!(retransmitted_fin.flags.ack());
    assert!(
...
fn tun_packet_loop_acknowledges_client_fin_after_server_fin_without_reset() {
    ...
    assert_eq!(summary.tcp_sessions_peak, 1);
    assert_eq!(summary.tcp_server_close_markers_peak, 1);
    assert_eq!(summary.tcp_post_close_markers_peak, 1);
    assert_eq!(device.writes.len(), 4);
    let server_fin = parse_tun_tcp_segment(&device.writes[2]).expect("parse server FIN packet");
    assert_eq!(server_fin.sequence_number, 1001);
    assert_eq!(server_fin.acknowledgment_number, 16);
...
#[test]
fn tun_packet_loop_acknowledges_post_close_fin_payload_without_reset() {
    let packets = vec![
        ipv4_packet(
            6,
            "10.7.0.2",
    ...
    .expect("run TUN loop with post-close FIN payload");
    ...
    assert!(server_fin.flags.fin());
    assert!(server_fin.flags.ack());
    let late_fin_payload_ack =
        parse_tun_tcp_segment(&device.writes[3]).expect("parse late FIN payload ACK packet");
    assert_eq!(late_fin_payload_ack.sequence_number, 1002);
    assert_eq!(late_fin_payload_ack.acknowledgment_number, 21);
    assert!(late_fin_payload_ack.flags.ack());
    assert!(!late_fin_payload_ack.flags.fin());
    assert!(!late_fin_payload_ack.flags.rst());
    assert!(late_fin_payload_ack.payload.is_empty());
    let duplicate_late_fin_payload_ack =
        parse_tun_tcp_segment(&device.writes[4]).expect("parse duplicate late FIN payload ACK");
    assert_eq!(duplicate_late_fin_payload_ack.sequence_number, 1002);
    assert_eq!(duplicate_late_fin_payload_ack.acknowledgment_number, 21);
    assert!(duplicate_late_fin_payload_ack.flags.ack());
...
#[test]
fn tun_packet_loop_duplicate_client_fin_with_latest_server_ack_polls_payload() {
    let packets = vec![
        ipv4_packet(
            6,
            "10.7.0.2",
    ...
    .expect("run TUN loop with latest-ACK duplicate FIN server payload poll");
    ...
    assert!(syn_ack.flags.syn());
    assert!(syn_ack.flags.ack());
    let client_payload_ack =
        parse_tun_tcp_segment(&device.writes[1]).expect("parse client payload ACK");
    assert_eq!(client_payload_ack.sequence_number, 1001);
    assert_eq!(client_payload_ack.acknowledgment_number, 16);
    assert!(client_payload_ack.flags.ack());
    assert!(client_payload_ack.payload.is_empty());
    ...
    let stale_fin_ack =
        parse_tun_tcp_segment(&device.writes[3]).expect("parse stale-ACK client FIN ACK");
    assert_eq!(stale_fin_ack.sequence_number, 1009);
    assert_eq!(stale_fin_ack.acknowledgment_number, 17);
    assert!(stale_fin_ack.flags.ack());
    assert!(!stale_fin_ack.flags.fin());
    assert!(!stale_fin_ack.flags.rst());
    assert!(stale_fin_ack.payload.is_empty());
    let duplicate_fin_ack =
        parse_tun_tcp_segment(&device.writes[4]).expect("parse latest-ACK duplicate FIN ACK");
    assert_eq!(duplicate_fin_ack.sequence_number, 1009);
    assert_eq!(duplicate_fin_ack.acknowledgment_number, 17);
    assert!(duplicate_fin_ack.flags.ack());
    assert!(!duplicate_fin_ack.flags.fin());
    assert!(!duplicate_fin_ack.flags.rst());
    assert!(duplicate_fin_ack.payload.is_empty());
    let second_server_payload =
        parse_tun_tcp_segment(&device.writes[5]).expect("parse second server payload");
