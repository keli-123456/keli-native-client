...
#[test]
fn tls_profiles_are_rejected_until_tls_transport_is_implemented() {
    ...
    .expect_err("tls profile should be explicit unsupported");

    assert_eq!(
        error,
        OutboundProfileError::UnsupportedSecurity {
            tag: "proxy".to_string(),
            security: SecurityKind::Tls {
                sni: Some("edge.example".to_string()),
                skip_verify: false,
