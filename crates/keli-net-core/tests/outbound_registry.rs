...
#[test]
fn socks5_outbound_uses_injected_dns_policy_for_proxy_server() {
    let mut registry = OutboundRegistry::new();
    registry.add_socks5_tcp(
        "proxy",
        Socks5TcpOutbound::new(Endpoint::new("127.0.0.1", 1080), ""),
    );
    let mut dns = ipv6_only_dns();
    ...
    let error = registry
        ...
        .expect_err("IPv6-only DNS policy should reject IPv4 proxy server");
...
#[test]
fn socks5_udp_outbound_uses_injected_dns_policy_for_proxy_server() {
    let mut registry = OutboundRegistry::new();
    registry.add_socks5_tcp(
        "proxy",
        Socks5TcpOutbound::new(Endpoint::new("127.0.0.1", 1080), ""),
    ...
    let error = registry
        ...
        .expect_err("IPv6-only DNS policy should reject IPv4 UDP proxy server");
...
#[test]
fn http_connect_outbound_uses_injected_dns_policy_for_proxy_server() {
    let mut registry = OutboundRegistry::new();
    registry.add_http_connect(
        "proxy",
        HttpConnectOutbound::new(Endpoint::new("127.0.0.1", 8080), ""),
    );
    let mut dns = ipv6_only_dns();
...
#[test]
fn h2_tcp_outbounds_use_injected_dns_policy_for_server() {
    assert_registered_tcp_uses_injected_dns_policy(
        registry_with(|registry| {
            registry.add_trojan_h2(
                "proxy",
                TrojanH2Outbound::new(loopback_proxy_endpoint(), "edge.example", "/h2", "password"),
            )
        }),
        "IPv6-only DNS policy should reject IPv4 Trojan H2 server",
    );
    assert_registered_tcp_uses_injected_dns_policy(
        registry_with(|registry| {
            registry.add_trojan_tls_h2(
                "proxy",
                TrojanTlsH2Outbound::new(
                    loopback_proxy_endpoint(),
                    "edge.example",
                    "/h2",
                    "password",
        ...
        "IPv6-only DNS policy should reject IPv4 Trojan TLS H2 server",
    );

    assert_registered_tcp_uses_injected_dns_policy(
        registry_with(|registry| {
            registry.add_vless_h2(
                "proxy",
                VlessH2Outbound::new(
                    loopback_proxy_endpoint(),
                    "edge.example",
        ...
        "IPv6-only DNS policy should reject IPv4 VLESS H2 server",
    );
    assert_registered_tcp_uses_injected_dns_policy(
        registry_with(|registry| {
            registry.add_vless_tls_h2(
                "proxy",
        ...
        "IPv6-only DNS policy should reject IPv4 VLESS TLS H2 server",
    );

    assert_registered_tcp_uses_injected_dns_policy(
        registry_with(|registry| {
            registry.add_vmess_h2(
                "proxy",
                VmessH2Outbound::new(loopback_proxy_endpoint(), "edge.example", "/h2", TEST_UUID),
            )
        }),
        "IPv6-only DNS policy should reject IPv4 VMess H2 server",
    );
    assert_registered_tcp_uses_injected_dns_policy(
        registry_with(|registry| {
            registry.add_vmess_tls_h2(
                "proxy",
                VmessTlsH2Outbound::new(
                    loopback_proxy_endpoint(),
                    "edge.example",
                    "/h2",
                    TEST_UUID,
        ...
        "IPv6-only DNS policy should reject IPv4 VMess TLS H2 server",
    );
}

#[test]
fn h2_udp_outbounds_use_injected_dns_policy_for_server() {
    assert_registered_udp_uses_injected_dns_policy(
        registry_with(|registry| {
            registry.add_trojan_h2(
                "proxy",
        ...
        "IPv6-only DNS policy should reject IPv4 Trojan H2 UDP server",
    );
    assert_registered_udp_uses_injected_dns_policy(
        registry_with(|registry| {
            registry.add_trojan_tls_h2(
        ...
        "IPv6-only DNS policy should reject IPv4 Trojan TLS H2 UDP server",
    );
    ...
    assert_registered_udp_uses_injected_dns_policy(
        ...
        "IPv6-only DNS policy should reject IPv4 VLESS H2 UDP server",
    );
    assert_registered_udp_uses_injected_dns_policy(
        registry_with(|registry| {
            registry.add_vless_tls_h2(
        ...
        "IPv6-only DNS policy should reject IPv4 VLESS TLS H2 UDP server",
    );
    ...
    assert_registered_udp_uses_injected_dns_policy(
        ...
        "IPv6-only DNS policy should reject IPv4 VMess H2 UDP server",
    );
    assert_registered_udp_uses_injected_dns_policy(
        registry_with(|registry| {
            registry.add_vmess_tls_h2(
        ...
        "IPv6-only DNS policy should reject IPv4 VMess TLS H2 UDP server",
    );
}
...
#[test]
fn registry_from_trojan_httpupgrade_profile_relays_udp_after_upgrade() {
    let listener = std::net::TcpListener::bind("127.0.0.1:0").expect("bind trojan hu udp server");
    let port = listener.local_addr().expect("trojan hu udp addr").port();
    let server = std::thread::spawn(move || {
        let (mut stream, _) = listener.accept().expect("accept trojan hu udp server");
        let request = read_http_request(&mut stream);
        assert_httpupgrade_request(&request, "/trojan-upgrade", "edge.example");
        stream
            .write_all(httpupgrade_response().as_bytes())
    ...
    let response = registry
        ...
        .expect("registered Trojan HTTPUpgrade UDP outbound should relay");
...
#[test]
fn registry_from_vless_ws_profile_relays_udp_over_websocket() {
    let listener = std::net::TcpListener::bind("127.0.0.1:0").expect("bind vless ws udp server");
    ...
    let response = registry
        ...
        .expect("registered VLESS WS UDP outbound should relay");
...
#[test]
fn registry_from_vless_httpupgrade_profile_relays_udp_after_upgrade() {
    let listener = std::net::TcpListener::bind("127.0.0.1:0").expect("bind vless hu udp server");
    let port = listener.local_addr().expect("vless hu udp addr").port();
    let server = std::thread::spawn(move || {
        let (mut stream, _) = listener.accept().expect("accept vless hu udp server");
        let request = read_http_request(&mut stream);
        assert_httpupgrade_request(&request, "/vless-upgrade", "edge.example");
        stream
            .write_all(httpupgrade_response().as_bytes())
    ...
    let response = registry
        ...
        .expect("registered VLESS HTTPUpgrade UDP outbound should relay");
...
#[test]
fn registry_from_vmess_tcp_profile_relays_udp_over_aes_gcm_chunks() {
    let uuid = "00112233-4455-6677-8899-aabbccddeeff";
    let listener = std::net::TcpListener::bind("127.0.0.1:0").expect("bind vmess udp server");
    ...
    let response = registry
        ...
        .expect("registered VMess UDP outbound should relay");
...
fn read_vmess_aead_request(stream: &mut impl Read, uuid: &str) -> VmessRequestForTest {
    ...
    let (target_host, consumed) = match header[40] {
        0x01 => {
            let ip = std::net::Ipv4Addr::new(header[41], header[42], header[43], header[44]);
            (ip.to_string(), 5)
        }
        0x02 => {
        ...
        other => panic!("unsupported test VMess address type {other}"),
    };
    assert!(40 + consumed <= header.len());
