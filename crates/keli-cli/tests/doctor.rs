...
fn doctor_report_lists_supported_outbounds() {
    ...
    assert!(output.contains(
        "readiness_check_capabilities=doctor-schema,interop-matrix,local-mixed-soak,resource-limits,tun-preflight,system-proxy,panel-subscription-state,support-diagnostics,json-gates,blocker-summary,soak-min-duration,tun-preflight-evidence,tun-runtime-smoke,tun-runtime-smoke-min-duration"
    ));
    assert!(output.contains(
