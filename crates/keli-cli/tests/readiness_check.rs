...
fn readiness_check_json_reports_default_core_gates_with_skipped_soak() {
    ...
    assert_eq!(report["summary"]["total_gate_count"], 73);
    ...
    assert_eq!(report["route_rule_smoke"]["case_count"], 5);
    ...
    let outbound_route_case = route_cases
        .iter()
        .find(|case| case["name"] == "domain-suffix-outbound-http-connect")
        .expect("outbound route smoke case");
    assert_eq!(outbound_route_case["expected_route_action"], "outbound");
    assert_eq!(outbound_route_case["observed_route_action"], "outbound");
    assert_eq!(
        outbound_route_case["expected_selected_outbound"],
        "ROUTE-RULE-SS-TCP-SMOKE"
    );
    assert_eq!(
        outbound_route_case["observed_selected_outbound"],
        "ROUTE-RULE-SS-TCP-SMOKE"
    );
    assert_eq!(outbound_route_case["outbound_confirmed"], true);
    assert_eq!(outbound_route_case["round_trip_observed"], true);
    assert_eq!(outbound_route_case["server_received_payload"], true);
    assert_eq!(
        outbound_route_case["observed_response"],
        "keli-route-rule-pong"
    );
    ...
    assert_hy2_quic_udp_relay_smoke_json(&report);
    ...
    assert_tuic_quic_udp_relay_smoke_json(&report);
...
fn readiness_check_text_reports_gate_summary() {
    ...
    assert!(output.contains(&format!(
        "readiness status=not-ready schema_version={} gates=73",
    ...
    assert!(output.contains("readiness route_rule_smoke status=passed cases=5"));
    ...
    assert!(output.contains("readiness hy2_quic_udp_relay_smoke status=passed cases=4"));
    assert!(output.contains("readiness tuic_quic_tcp_relay_smoke status=passed cases=4"));
    assert!(output.contains("readiness tuic_quic_udp_relay_smoke status=passed cases=4"));
...
fn default_core_certification_json_embeds_readiness_and_backend_evidence() {
    ...
    assert_eq!(report["readiness"]["route_rule_smoke"]["case_count"], 5);
    ...
    assert_eq!(
        report["certification"]["hy2_quic_udp_relay_smoke_passed"],
        true
    );
    assert_hy2_quic_udp_relay_smoke_json(&report);
    assert_eq!(
        report["readiness"]["hy2_quic_udp_relay_smoke"]["status"],
        "passed"
    );
    assert_eq!(
        report["readiness"]["hy2_quic_udp_relay_smoke"]["case_count"],
        4
    );
    assert_eq!(
    ...
    assert_eq!(
        report["certification"]["tuic_quic_udp_relay_smoke_passed"],
        true
    );
    assert_tuic_quic_udp_relay_smoke_json(&report);
    assert_eq!(
        report["readiness"]["tuic_quic_udp_relay_smoke"]["status"],
        "passed"
    );
    assert_eq!(
        report["readiness"]["tuic_quic_udp_relay_smoke"]["case_count"],
        4
    );
    assert_eq!(
...
fn default_core_certification_text_reports_summary_and_gates() {
    ...
    assert!(output.contains("default_core_certification route_rule_smoke status=passed cases=5"));
    ...
    assert!(output
        .contains("default_core_certification hy2_quic_udp_relay_smoke status=passed cases=4"));
    assert!(output
        .contains("default_core_certification tuic_quic_tcp_relay_smoke status=passed cases=4"));
    assert!(output
        .contains("default_core_certification tuic_quic_udp_relay_smoke status=passed cases=4"));
    assert!(
...
fn assert_hy2_quic_udp_relay_smoke_json(report: &Value) {
    assert_eq!(report["hy2_quic_udp_relay_smoke"]["status"], "passed");
    assert_eq!(report["hy2_quic_udp_relay_smoke"]["passed"], true);
    assert_eq!(report["hy2_quic_udp_relay_smoke"]["case_count"], 4);
    assert_eq!(report["hy2_quic_udp_relay_smoke"]["failed_case_count"], 0);
    assert_eq!(
        report["hy2_quic_udp_relay_smoke"]["selected_outbound"],
        "HY2-QUIC-UDP-SMOKE"
    );
    assert_eq!(
        report["hy2_quic_udp_relay_smoke"]["target"],
        "example.com:53"
    );
    assert!(report["hy2_quic_udp_relay_smoke"]["relay_port"].is_number());
    assert_eq!(
        report["hy2_quic_udp_relay_smoke"]["response_source"],
        "127.0.0.1:53"
    );
    assert_eq!(
        report["hy2_quic_udp_relay_smoke"]["request_payload_bytes"],
        18
    );
    assert_eq!(
        report["hy2_quic_udp_relay_smoke"]["response_payload_bytes"],
        17
    );
    assert_eq!(
        report["hy2_quic_udp_relay_smoke"]["round_trip_observed"],
        true
    );
    assert_eq!(
        report["hy2_quic_udp_relay_smoke"]["server_received_payload"],
        true
    );
    assert_eq!(report["hy2_quic_udp_relay_smoke"]["metrics_recorded"], true);
    assert_eq!(
        report["hy2_quic_udp_relay_smoke"]["metrics_inbound_count"],
        1
    );
    assert_eq!(
        report["hy2_quic_udp_relay_smoke"]["metrics_outbound_route_count"],
        1
    );
    assert_eq!(
        report["hy2_quic_udp_relay_smoke"]["clean_stop_observed"],
        true
    );
    assert_eq!(
        report["hy2_quic_udp_relay_smoke"]["stop_workers_remaining"],
        0
    );
    assert_eq!(report["hy2_quic_udp_relay_smoke"]["stop_timed_out"], false);

    let cases = report["hy2_quic_udp_relay_smoke"]["cases"]
        .as_array()
        .expect("HY2 QUIC UDP relay smoke cases");
    let case_names: Vec<_> = cases
        .iter()
        .filter_map(|case| case["name"].as_str())
        .collect();
    ...
    let round_trip = cases
        ...
        .expect("HY2 QUIC UDP relay round trip case");
    assert_eq!(round_trip["observed_response"], "keli-hy2-udp-pong");
    assert_eq!(round_trip["response_source"], "127.0.0.1:53");
    assert_eq!(round_trip["round_trip_observed"], true);
    assert_eq!(round_trip["server_received_payload"], true);
}

fn assert_tuic_quic_udp_relay_smoke_json(report: &Value) {
    assert_eq!(report["tuic_quic_udp_relay_smoke"]["status"], "passed");
    assert_eq!(report["tuic_quic_udp_relay_smoke"]["passed"], true);
    assert_eq!(report["tuic_quic_udp_relay_smoke"]["case_count"], 4);
    assert_eq!(report["tuic_quic_udp_relay_smoke"]["failed_case_count"], 0);
    assert_eq!(
        report["tuic_quic_udp_relay_smoke"]["selected_outbound"],
        "TUIC-QUIC-UDP-SMOKE"
    );
    assert_eq!(
        report["tuic_quic_udp_relay_smoke"]["target"],
        "example.com:53"
    );
    assert!(report["tuic_quic_udp_relay_smoke"]["relay_port"].is_number());
    assert_eq!(
        report["tuic_quic_udp_relay_smoke"]["response_source"],
        "127.0.0.1:53"
    );
    assert_eq!(
        report["tuic_quic_udp_relay_smoke"]["request_payload_bytes"],
        19
    );
    assert_eq!(
        report["tuic_quic_udp_relay_smoke"]["response_payload_bytes"],
        18
    );
    assert_eq!(
        report["tuic_quic_udp_relay_smoke"]["round_trip_observed"],
        true
    );
    assert_eq!(
        report["tuic_quic_udp_relay_smoke"]["server_received_payload"],
        true
    );
    assert_eq!(
        report["tuic_quic_udp_relay_smoke"]["metrics_recorded"],
        true
    );
    assert_eq!(
        report["tuic_quic_udp_relay_smoke"]["metrics_inbound_count"],
        1
    );
    assert_eq!(
        report["tuic_quic_udp_relay_smoke"]["metrics_outbound_route_count"],
        1
    );
    assert_eq!(
        report["tuic_quic_udp_relay_smoke"]["clean_stop_observed"],
        true
    );
    assert_eq!(
        report["tuic_quic_udp_relay_smoke"]["stop_workers_remaining"],
        0
    );
    assert_eq!(report["tuic_quic_udp_relay_smoke"]["stop_timed_out"], false);

    let cases = report["tuic_quic_udp_relay_smoke"]["cases"]
        .as_array()
        .expect("TUIC QUIC UDP relay smoke cases");
    let case_names: Vec<_> = cases
        .iter()
        .filter_map(|case| case["name"].as_str())
        .collect();
    for expected in [
        "start-tuic-quic-udp-relay-runtime",
        "tuic-quic-udp-protocol-round-trip",
        "record-tuic-quic-udp-relay-metrics",
        "stop-tuic-quic-udp-relay-runtime",
    ] {
        assert!(
            case_names.contains(&expected),
            "missing TUIC QUIC UDP relay smoke case {expected}: {case_names:?}"
        );
    }
    let round_trip = cases
        .iter()
        .find(|case| case["name"] == "tuic-quic-udp-protocol-round-trip")
        .expect("TUIC QUIC UDP relay round trip case");
    assert_eq!(round_trip["observed_response"], "keli-tuic-udp-pong");
    assert_eq!(round_trip["response_source"], "127.0.0.1:53");
    assert_eq!(round_trip["round_trip_observed"], true);
    assert_eq!(round_trip["server_received_payload"], true);
}
