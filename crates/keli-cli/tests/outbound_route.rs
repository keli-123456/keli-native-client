...
use keli_net_core::{
    websocket_accept_for_key, DnsAddressFamilyPolicy, OutboundRegistry, RouteAction, RouteEngine,
    TrojanWsOutbound, VlessWsOutbound,
};
...
#[test]
fn registered_direct_outbound_route_respects_dns_policy() {
    let target = TcpListener::bind("127.0.0.1:0").expect("bind target");
    let target_port = target.local_addr().expect("target addr").port();
    target
    ...
    let mut outbounds = OutboundRegistry::new();
    outbounds.add_direct("proxy");
    let mut runtime = MixedProxyRuntime::with_routes_and_outbounds(
        RouteEngine::new(RouteAction::Outbound("proxy".to_string())),
        outbounds,
    );
    ...
    let inbound_thread = thread::spawn(move || {
        ...
        handle_mixed_connection_with_routes(&mut stream, &runtime)
            .expect_err("registered direct outbound should use runtime DNS policy");
    });
    ...
    let mut connect_response = Vec::new();
    read_until_header_end(&mut client, &mut connect_response);
    assert_eq!(connect_response, b"HTTP/1.1 400 Bad Request\r\n\r\n");

    inbound_thread.join().expect("inbound thread");
    assert!(
        target.accept().is_err(),
        "DNS address-family policy must prevent registered direct outbound connections"
    );
}
