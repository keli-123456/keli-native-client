...
5. Production readiness: real interop matrix, long soak tests, resource limits,
   ...
   `socks5`/outbound metrics, and stops cleanly. The default
   ...
   smoke evidence, resource-limit smoke evidence, panel/subscription smoke evidence, runtime recovery smoke evidence, TUN TCP session smoke evidence, TUN TCP session limit smoke evidence, TUN TCP session idle-prune smoke evidence, certification
   ...
   host TUN adapter. The default TUN TCP session limit smoke is also always
   part of readiness and certification, proving the managed packet loop
   enforces the max-active-session guard, records one
   `TcpSessionLimitExceeded` rejection, keeps bounded active-session counters
   visible, and needs no host TUN adapter. The default TUN TCP session
   idle-prune smoke is likewise always part of readiness and certification,
   ...
   counters without host TUN access. The optional TUN runtime smoke gate is also carried into certification JSON,
