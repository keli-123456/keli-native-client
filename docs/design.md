...
3. Client DNS and TUN: TUN device lifecycle, DNS hijack, leak prevention,
   ...
   resolution. Core TCP outbounds for Trojan, VLESS, Shadowsocks, VMess
   plain/TLS TCP, and Trojan/VLESS/VMess WebSocket and HTTPUpgrade transports
   also use the injected DNS policy for server resolution.
